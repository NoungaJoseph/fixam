...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('const sendWelcomeEmail = async')) {
    skip = true;
    newLines.push(`const sendWelcomeEmail = async (email, fullName, language = 'en') => {
...
const sendSuspiciousLoginAlert = async (email, details, language = 'en') => {
  ...
  if (skip && lines[i].includes('const sendMarketingBroadcast = async')) {
    skip = false;
  }
